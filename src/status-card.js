...
class StatusCard extends BaseCard {
  ...
  setConfig(config) {
      ...
      this._entityConfig = this.initializeEntityConfig();
  }
  ...
  getProperty(group, propertyType) {
    ...
    if (this._propertyCache.has(cacheKey)) {
      const cachedValue = this._propertyCache.get(cacheKey);
      if (propertyType === 'status' && group.entity_id && group.entity_id.startsWith('person.')) {
        ...
        if (entityState.state !== cachedValue) {
          this._propertyCache.delete(cacheKey);
    ...
    const entityConfig = this._entityConfig[type];
    ...
    if (propertyType === 'sortOrder') {
      if (type === 'extra') {
        result = this._config.newSortOrder?.['extra']?.[deviceClassName] ?? -1;
      } else {
    ...
    this._propertyCache.set(cacheKey, result);
    return result;
  }
...
toggleMoreInfo(action, domain = null, entities = null, entityName = null) {
  ...
  updateFilteredAreasAndDevices() {
    const { area, floor } = this._config.area_filter || {};
  ...
  loadGroupedEntities() {
    ...
    const groupedEntities = this.entities.reduce((acc, entity) => {
      ...
      if (!this._entityConfig[domain] || !state || UNAVAILABLE_STATES.has(state) || this.isEntityHidden(entity)) return acc;
  ...
  loadExtraEntities() {
    return (this._config?.extra_entities || []).reduce((acc, { entity, status, icon, color }) => {
      ...
      if (selectedEntity && selectedEntity.state === status) {
        acc.push({
          ...
          color,
          device_class: entity,
          sortOrder: this._config.newSortOrder?.['extra']?.[entity] ?? -1
        });
      }
      return acc;
    }, []);
        ...
        <paper-tab @click=${() => this.showMoreInfo(entity)}>
          <div class="entity">
            ...
            <div class="entity-info">
              <div class="entity-name">${cachedData.friendlyName}</div>
...
class StatusCardEditor extends BaseCard {
  ...
  setConfig(config) {
    this._config = config;
    if (!config.extra_entities) delete this._config.extra_entities;
    if (!config.hidden_entities) delete this._config.hidden_entities;
  }
...
renderSettings() {
  ...
  const selectedAreaFloor = this._config.area_filter ? Object.keys(this._config.area_filter)[0] : '';
  ...
  return html`
    <div class="settings">
      <div class="top-row">
        ...
        <div class="toggle">
          ${this.renderHaForm("showPerson", { boolean: {} }, { showPerson: this._config.showPerson ?? true }, this._computeLabel, 
            (e) => this.toggleSetting('showPerson', e.detail.value.showPerson))}
          ${this.renderHaForm("bulkMode", { boolean: {} }, { bulkMode: this._config.bulkMode ?? false }, this._computeLabel, 
          ...
          .map(domain => html`
            <ha-expansion-panel outlined ?expanded="${this.openedDomains.includes(domain)}" class="child">
              ...
              <div class="content" style="padding: 10px 5px;">
                ${this.renderHaForm("hideDomain", { boolean: {} }, { hideDomain: this._config.hide?.[domain] || false }, 
                  (schema) => this._computeLabel(schema, domain), 
                  (e) => this.updateVisibility(domain, null, e.detail.value.hideDomain))}
                ${this.renderHaForm("domainName", { text: {} }, { domainName: this._config.names?.[domain] || '' }, this._computeLabel, 
                  (e) => this.updateNames(domain, null, e.detail.value.domainName))}
                ${this.renderHaForm("icon", { icon: {} }, { icon: this._config.icons?.[domain] || '' }, this._computeLabel, 
                  (e) => this.updateIcons(domain, null, e.detail.value.icon))}
                ${this.renderHaForm("color", { ui_color: {} }, { color: this._config.colors?.[domain] || '' }, this._computeLabel, 
                  (e) => this.updateColors(domain, null, e.detail.value.color))}
                ${this.renderHaForm("sortOrder", { number: {} }, { sortOrder: this._config.newSortOrder?.[domain] !== undefined ? this._config.newSortOrder?.[domain] : this.entityConfig[domain]?.sortOrder || 0 }, this._computeLabel, (e) => { const newValue = Number(e.detail.value.sortOrder); this.newSortOrder = newValue; this.updateSortOrder(domain, null, newValue); })}
              </div>
            </ha-expansion-panel>
          `)}
      </div>
            ...
            <ha-expansion-panel outlined ?expanded="${this.openedDeviceClasses.includes(`${domain}-${deviceClass.name}`)}" class="child">
              ...
              <div class="content" style="padding: 10px 5px;">
                ${this.renderHaForm(`hide_${domain}_DeviceClass`, { boolean: {} }, 
                  { [`hide_${domain}_DeviceClass`]: this._config.hide?.[domain]?.[deviceClass.name] || false }, 
                ...
                ${this.renderHaForm("deviceClassName", { text: {} }, 
                  { deviceClassName: this._config.names?.[domain]?.[deviceClass.name] || '' }, this._computeLabel, 
                  (e) => this.updateNames(domain, deviceClass.name, e.detail.value.deviceClassName))}
                ${this.renderHaForm("icon", { icon: {} }, 
                  { icon: this._config.icons?.[domain]?.[deviceClass.name] || '' }, this._computeLabel, 
                  (e) => this.updateIcons(domain, deviceClass.name, e.detail.value.icon))}
                ${this.renderHaForm("color", { ui_color: {} }, 
                  { color: this._config.colors?.[domain]?.[deviceClass.name] || '' }, this._computeLabel, 
                  (e) => this.updateColors(domain, deviceClass.name, e.detail.value.color))}
                ${this.renderHaForm("sortOrder", { number: {} }, { sortOrder: (this._config.newSortOrder?.[domain]?.[deviceClass.name] !== undefined) ? this._config.newSortOrder[domain][deviceClass.name] : deviceClass.sortOrder || 0 }, this._computeLabel, (e) => this.updateSortOrder(domain, deviceClass.name, Number(e.detail.value.sortOrder)))}
              </div>
            </ha-expansion-panel>
          `)}
        </div>
      ...
      <div class="extra-entity-bottom">
        ...
        <div class="extra-entity-sort">
          ${this.renderHaForm("sortOrder", { number: {} }, 
            { sortOrder: this._config.newSortOrder?.['extra']?.[entity.entity] ?? '' }, this._computeLabel, 
...
renderHiddenEntities() {
  const hiddenEntities = (this._config.hidden_entities || []).slice().sort();
